...
class Ship:
    ...
    def draw(self, win):
        if self.is_invulneravel():
            if (self.invulneravel_timer // 5) % 2 == 0:
                return  # piscar
...
def game_over_screen(score):
    ...
    while True:
        ...
        score_text = font_small.render(f"Pontuação final: {score}", True, WHITE)
        restart_text = font_small.render("Pressione R para reiniciar ou ESC para sair", True, WHITE)
    ...
    while running:
        ...
        for asteroid in asteroids[:]:
            if ship.get_rect().colliderect(asteroid.get_rect()) and not ship.is_invulneravel():
                asteroids.remove(asteroid)
                vidas -= 1
                ship.invulneravel_timer = 60  # 1 segundo de invulnerabilidade
                if vidas <= 0:
